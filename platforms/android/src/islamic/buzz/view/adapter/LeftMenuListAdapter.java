...
public class LeftMenuListAdapter extends ArrayAdapter<MenuCategory> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.textMenuItem = (TextView) convertView.findViewById(R.id.id_navigationDrawer_menuItemTxt);

			holder.imageViewItem = (ImageView) convertView.findViewById(R.id.id_navigationDrawer_imageView);

			holder.progressBar = (ProgressBar) convertView.findViewById(R.id.id_navigationDrawer_progress);

			holder.listDivider = (ImageView) convertView.findViewById(R.id.id_navigationDrawer_divider);
		...
		if (categoryList.get(position).getImage() != null && categoryList.get(position).getImage().length() > 0) {

			Drawable imageDrawable = getContext().getResources().getDrawable(getContext().getResources().getIdentifier(categoryList.get(position).getImage(), "drawable", getContext().getPackageName()));
			...
			int paddingLeft = (int) (getContext().getResources().getDimension(R.dimen.navigation_drawer_text_paddingLeft) / getContext().getResources().getDisplayMetrics().density);

			holder.imageViewItem.setPadding(paddingLeft, holder.imageViewItem.getPaddingTop(), holder.imageViewItem.getPaddingRight(), holder.imageViewItem.getPaddingBottom());
			...
			if (categoryList.get(position).getName().equalsIgnoreCase(getContext().getString(R.string.home)) || categoryList.get(position).getType() == MenuCategory.TYPE_BOLD || categoryList.get(position).getName().equalsIgnoreCase(getContext().getString(R.string.home))) {
			...
			if (categoryList.get(position).getName().equalsIgnoreCase(getContext().getString(R.string.home))) {

				// /Set the top row height of the menu equal to the height of
				// the action bar.
				...
				// Show Home Image
				holder.textMenuItem.setCompoundDrawablesWithIntrinsicBounds(R.drawable.home_white, 0, 0, 0);

				// Calcuate the padding on the left, since there is an image,
				// the padding is different from the non image textview ones.
				paddingLeft = (int) (getContext().getResources().getDimension(R.dimen.navigation_drawer_texthomeimage_paddingLeft) / getContext().getResources().getDisplayMetrics().density);

				// Home has different textsize
				holder.textMenuItem.setTextSize(getContext().getResources().getDimension(R.dimen.navigation_drawer_home_text_size) / getContext().getResources().getDisplayMetrics().density);
			} // end if
			else if (categoryList.get(position).getName().equalsIgnoreCase(getContext().getString(R.string.home))) {
				// Show Home Image
				holder.textMenuItem.setCompoundDrawablesWithIntrinsicBounds(R.drawable.back_icon, 0, 0, 0);

				// Calcuate the padding on the left, since there is an image,
				// the padding is different from the non image textview ones.
				paddingLeft = (int) (getContext().getResources().getDimension(R.dimen.navigation_drawer_textbackimage_paddingLeft) / getContext().getResources().getDisplayMetrics().density);
			} // end Else if
			else {
				holder.textMenuItem.setCompoundDrawablesWithIntrinsicBounds(0, 0, 0, 0);
				holder.textMenuItem.setTextSize(getContext().getResources().getDimension(R.dimen.navigation_drawer_text_size) / getContext().getResources().getDisplayMetrics().density);

				paddingLeft = (int) (getContext().getResources().getDimension(R.dimen.navigation_drawer_text_paddingLeft) / getContext().getResources().getDisplayMetrics().density);
			}

			holder.textMenuItem.setPadding(paddingLeft, holder.textMenuItem.getPaddingTop(), holder.textMenuItem.getPaddingRight(), holder.textMenuItem.getPaddingBottom());
