...
import com.jeremyfeinstein.slidingmenu.lib.app.SlidingMenu.CanvasTransformer;
...
public class CustomViewBehind extends ViewGroup {
    ...
    public void scrollBehindTo(View content,
            int x,
            int y) {
        ...
        if (mMode == SlidingMenu.LEFT) {
            ...
            if (x <= content.getLeft())
                vis = View.INVISIBLE;
            scrollTo((int) (getBehindWidth() - getWidth() + (x - getBehindWidth()) * mScrollScale),
                    y);
    ...
    public int getMenuLeft(View content,
            int page) {
        if (mMode == SlidingMenu.LEFT) {
            switch (page) {
                case 0:
    ...
    public boolean marginTouchAllowed(View content,
            int x) {
    ...
    public boolean menuOpenTouchAllowed(View content,
            int currPage,
            float x) {
        switch (mTouchMode) {
    ...
    public boolean menuTouchInQuickReturn(View content,
            int currPage,
            float x) {
    ...
    public void drawShadow(View content,
            Canvas canvas) {
    ...
    public void drawFade(View content,
            Canvas canvas,
            float openPercent) {
        if (!mFadeEnabled)
            return;
    ...
    public void drawSelector(View content,
            Canvas canvas,
            float openPercent) {
        if (!mSelectorEnabled)
            return;
        if (mSelectorDrawable != null && mSelectedView != null) {
            ...
            if (tag.equals(TAG + "SelectedView")) {
                ...
                if (mMode == SlidingMenu.LEFT) {
                    ...
                    right = left + offset;
                    canvas.clipRect(left, 0, right, getHeight());
                    canvas.drawBitmap(mSelectorDrawable,
                            right - mSelectorDrawable.getWidth(),
                            getSelectorTop(),
                            null);
                }
                canvas.restore();
