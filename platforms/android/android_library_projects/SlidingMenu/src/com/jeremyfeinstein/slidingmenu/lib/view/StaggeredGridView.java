...
public class StaggeredGridView extends AdapterView<ListAdapter> {
    ...
    private static final class LayoutRecord {
        ...
        public final void setMarginAbove(int col,
                int margin) {
        ...
        public final void setMarginBelow(int col,
                int margin) {
            if (mMargins == null && margin == 0) {
                return;
            }
        ...
        public String toString() {
            String result =
                    "LayoutRecord{c=" + column + ", id=" + id + " h=" + height + " s=" + span;
            if (mMargins != null) {
    ...
    public StaggeredGridView(Context context,
        ...
        if (attrs != null) {
            TypedArray a =
                    getContext().obtainStyledAttributes(attrs, R.styleable.CostomStaggeredGridView);
            mColCountSetting = a.getInteger(R.styleable.CostomStaggeredGridView_columnNumber, 2);
            mDrawSelectorOnTop =
                    a.getBoolean(R.styleable.CostomStaggeredGridView_drawSelectorOnTop, false);
            if (mColCountSetting != -1) {
                mColCount = mColCountSetting;
            }
            mMinColWidth =
                    a.getDimensionPixelSize(R.styleable.CostomStaggeredGridView_minColumnWidth, 0);
            a.recycle();
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (index < 0) {
                    Log.e(TAG,
                            "onInterceptTouchEvent could not find pointer with id " + mActivePointerId
                ...
                switch (prevTouchMode) {
                    ...
                    case TOUCH_MODE_DONE_WAITING:
                        ...
                        final boolean inList =
                                x > getPaddingLeft() && x < getWidth() - getPaddingRight();
    ...
    protected void onMeasure(int widthMeasureSpec,
        ...
        if (widthMode != MeasureSpec.EXACTLY) {
            Log.e(TAG,
                    "onMeasure: must have an exact width or match_parent! " + "Using fallback spec of EXACTLY "
                            + widthSize);
            widthMode = MeasureSpec.EXACTLY;
        }
        if (heightMode != MeasureSpec.EXACTLY) {
            Log.e(TAG,
                    "onMeasure: must have an exact height or match_parent! " + "Using fallback spec of EXACTLY "
                            + heightSize);
    ...
    private void populate(boolean clearData) {
        ...
        for (int i = 0; i < colCount; i++) {
            if (mRestoreOffsets != null && i >= mRestoreOffsets.length) {
                continue;
            }
            final int offset =
                    top + ((mRestoreOffsets != null) ? Math.min(mRestoreOffsets[i], 0) : 0);
    ...
    final void layoutChildren(boolean queryAdapter) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            int childTop =
                    mItemBottoms[col] > Integer.MIN_VALUE ? mItemBottoms[col] + mItemMargin
                            : child.getTop();
            if (span > 1) {
                int lowest = childTop;
     ...
     *         items at the top
     */
    final int fillUp(int fromPosition,
            int overhang) {
        final int paddingLeft = getPaddingLeft();
        final int paddingRight = getPaddingRight();
        final int itemMargin = mItemMargin;
     ...
     *         the bottom
     */
    final int fillDown(int fromPosition,
            int overhang) {
        final int paddingLeft = getPaddingLeft();
        final int paddingRight = getPaddingRight();
        final int itemMargin = mItemMargin;
     ...
     * @return
     */
    final LayoutRecord getNextRecordUp(int position,
            int span) {
        LayoutRecord rec = mLayoutRecords.get(position);
        if (rec == null) {
            rec = new LayoutRecord();
    ...
    final LayoutRecord getNextRecordDown(int position,
            int span) {
        LayoutRecord rec = mLayoutRecords.get(position);
        if (rec == null) {
            rec = new LayoutRecord();
    ...
    final View obtainView(int position,
        ...
        final View scrap =
                optType == positionViewType ? optScrap : mRecycler.getScrapView(positionViewType);
    ...
    public static class LayoutParams extends ViewGroup.LayoutParams {
        ...
        public LayoutParams(int height) {
            ...
            if (this.height == MATCH_PARENT) {
                Log.w(TAG,
                        "Constructing LayoutParams with height FILL_PARENT - " + "impossible! Falling back to WRAP_CONTENT");
        ...
        public LayoutParams(Context c,
            ...
            if (this.height == MATCH_PARENT) {
                Log.w(TAG,
                        "Inflation setting LayoutParams height to MATCH_PARENT - " + "impossible! Falling back to WRAP_CONTENT");
                this.height = WRAP_CONTENT;
            }
        ...
        public LayoutParams(ViewGroup.LayoutParams other) {
            ...
            if (this.height == MATCH_PARENT) {
                Log.w(TAG,
                        "Constructing LayoutParams with height MATCH_PARENT - " + "impossible! Falling back to WRAP_CONTENT");
                this.height = WRAP_CONTENT;
            }
        }
    }
    ...
    public interface OnScrollListener {
         ...
         *            {@link #SCROLL_STATE_IDLE}.
         */
        public void onScrollStateChanged(ViewGroup view,
                int scrollState);
         ...
         */
        public void onScroll(ViewGroup view,
    ...
    void positionSelector(int position,
            View sel) {
    ...
    private void positionSelector(int l,
            int t,
            int r,
            int b) {
    ...
    boolean performLongPress(final View child,
        ...
        if (getOnItemLongClickListener() != null) {
            handled =
                    getOnItemLongClickListener().onItemLongClick(this,
                            child,
                            longPressPosition,
                            longPressId);
        }
     ...
     */
    ContextMenuInfo createContextMenuInfo(View view,
            int position,
            long id) {
