...
{
    public LeftMenuItem(Context context, int iconResId, String title,ImageView.ScaleType scaleType, int cornerRadius,int borderWidth, int borderColorStateList,boolean roundBackgroud, boolean isOval)
    {
	super(context);
	...
	RoundedImageView imageView = new RoundedImageView(context, iconResId,scaleType, cornerRadius, borderWidth, borderColorStateList, roundBackgroud, isOval);
	//imageView.setImageDrawable(getResources().getDrawable(iconResId));
	((LinearLayout)findViewById(R.id.image_view_holder)).addView(imageView);
